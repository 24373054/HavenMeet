...
class ZhiHuLogin(AbstractLogin):
    ...
    async def login_by_qrcode(self):
        ...
        # save qrcode to file
        if "," in base64_qrcode_img:
            qr_data = base64_qrcode_img.split(",")[1]
        else:
            qr_data = base64_qrcode_img
        import base64
        from pathlib import Path
        qr_path = Path("/home/Matrix/.openclaw/workspace/MediaCrawler-main/zhihu_qrcode.png")
        with open(qr_path, "wb") as f:
            f.write(base64.b64decode(qr_data))
        utils.logger.info(f"[ZhiHu.login_by_qrcode] QR code saved to {qr_path}")
